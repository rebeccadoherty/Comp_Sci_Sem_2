...
class starter {
		...
		public static void searchArrInt(ArrayList<Integer> arr, int a)
		{
			for(int x= 0; x < arr.size(); x++)
			{
				if(arr.get(x) == a)
				{
					System.out.println("integer does exist at index " + x);
				}
				else if((x == arr.size() - 1) && (arr.get(x) != a))
				{
					System.out.println("integer does not exist");
				}
			}
		}
		...
		public static void searchArrString(ArrayList<String> arr, String a)
		{
			...
			{
				if(arr.get(y).equals(a))
				{
					System.out.println("integer does exist at index " + y);
				}
